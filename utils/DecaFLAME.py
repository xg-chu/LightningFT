...
class FLAME(nn.Module):
    ...
    def __init__(self, flame_model_path, flame_lmk_embedding_path, n_shape, n_exp):
...
class FLAME_mediapipe(FLAME): 
    def __init__(
            self, flame_model_path, flame_lmk_embedding_path, 
            flame_mediapipe_lmk_embedding_path, flame_mediapipe_lmk_idx_path,
            n_shape, n_exp
        ):
        super().__init__(flame_model_path, flame_lmk_embedding_path, n_shape, n_exp)
        # static MEDIAPIPE landmark embeddings for FLAME
        lmk_embeddings_mediapipe = np.load(
            flame_mediapipe_lmk_embedding_path, allow_pickle=True, encoding='latin1'
        )
        lmk_idx_mediapipe = np.load(
            flame_mediapipe_lmk_idx_path, allow_pickle=True, encoding='latin1'
        )
        self.register_buffer('lmk_idx_mediapipe', 
            torch.tensor(lmk_idx_mediapipe.astype(np.int64), dtype=torch.long))
    ...
    def forward(self, shape_params=None, expression_params=None, pose_params=None, eye_pose_params=None):
        ...
        all_landmarks2d_mediapipe = vertices[:, self.lmk_idx_mediapipe]
        # landmarks3d_mediapipe = vertices2landmarks(
        #     vertices, self.faces_tensor,
        #     self.full_lmk_faces_idx_mediapipe.repeat(bz, 1),
        #     self.full_lmk_bary_coords_mediapipe.repeat(bz, 1, 1)
        # )
        return vertices, landmarks2d, landmarks3d, landmarks2d_mediapipe, all_landmarks2d_mediapipe # , landmarks3d_mediapipe
